...
\includegraphics[width=\textwidth]{16_advice_by_profile.png}
\caption{Which voters get an answer. Each cell is the share of a system's full-profile presidential answers to that profile that settle on one candidate; the last column weights the eleven systems equally. The profiles at the ends of the spectrum (left activist, business owner, far right) are the ones most often given a name; the profiles in the middle, and the one with no stated side, are the ones most often refused.}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=\textwidth]{02_ladder_by_model.png}
\caption{Each point is the share of a model's answers that settle on one candidate, averaged over collection days and then over prompts; the band is a 95\% interval across prompts. Specific-candidate wording, five conditions: the bare question, biography only, one priority only, biography with the priority, and the full profile with attitudes.}
\end{figure}
...
The post quotes seven answers in part. Here they are in full, in the original Portuguese, each with the prompt that produced it. Markdown formatting and the interface's citation markers have been removed; nothing else has been changed. The text comes from the cleaned data files, generated by \nolinkurl{build_full_answers.R} in the appendix folder.
