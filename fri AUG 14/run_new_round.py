...
pilot dataset. Reuses query()/client_for() from runners/run_api.py unmodified.

Model list for this round (10) — see fri AUG 14/next_run_plan.md for the reasoning
behind each pick. Two ids (gemini_31pro, grok_46) are inferred by OpenRouter's
naming pattern from the vendor's own release announcements, not yet confirmed
against a live OpenRouter listing — check with --dry-run / a 1-prompt test before
a full run, and swap in gemini_flash / grok's confirmed id if they 404.
...
# exist (gpt4o, claude_opus, claude_sonnet, gemini_pro, grok, sabia4, llama), adds
# the new GPT-5.6 variants and DeepSeek back in, and upgrades Claude/Gemini/Grok to
# their latest releases as of 2026-08-14 (Claude Opus 5 / Sonnet 5, Grok 4.6; Gemini
# left as the existing gemini_pro entry pending confirmation of a stable Gemini 3.1
# Pro id -- see note above).
ROUND_MODELS = {
    ...
    "gemini_pro":     PILOT_MODELS["gemini_pro"],  # TODO confirm google/gemini-3.1-pro availability, swap in if stable
